...
lstm_model = keras.models.load_model("trained_models/temporal_weights.h5")
lstm_extractor = keras.models.Model(
    inputs = lstm_model.input,
    outputs = lstm_model.get_layer("lstm_1").output
)
...
for i, vid_name in enumerate(normal_videos):
    ...
    features_clips = [cnn_extractor.predict(clip) for clip in prep_clips]
    features_clips = np.stack(features_clips, axis=0)

    features = lstm_extractor.predict(features_clips)
